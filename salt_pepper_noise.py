...
from file_manager import AzureStorageManager
...
def add_irregular_patch(
    noisy_image: np.array, patch_pixels: int, noise_value: int
) -> np.array:
    """
...
def apply_random_irregular_noise(
    image: np.array, amount: float, salt_vs_pepper: float
) -> np.array:
    """
...
def main(file_path: Path, amount: float, salt_vs_pepper: float):
    ...
    # save noisy image
    output_file_path = (
        Path("noisy_frames") / f"{file_path.stem}_noisy{file_path.suffix}"
    )
...
if __name__ == "__main__":
    ...
    manager.upload_files(
        local_files_dir=Path("noisy_frames"),
        azure_dir_to_upload="noisy_images",
        container_name="data",
        pattern="*.jpg",
    )
